class Renderer {
    ...
    updateTransforms(time, delta_time) {
        ...
        this.final_r = (this.final_r + this.final_vr*(delta_time/1000)) % (2*Math.PI);
        

        this.final_sx = (this.final_sx_m + (Math.cos(this.final_sv *time/1000)));
        this.final_sy = (this.final_sy_m + (Math.cos(this.final_sv *time/1000)));
    ...
    drawSlide3() {
        ...
        for (let i = 0; i < ball_points.length; i++){
            center.x = center.x + ball_points[i].values[0][0];
            
            center.y = center.y + ball_points[i].values[1][0];
        }
        center.x = parseInt(center.x/ball_points.length);
        center.y = parseInt(center.y/ball_points.length);


        mat3x3Translate(mover, - 50, - 50);
        mat3x3Rotate(rotateTransform, this.final_r); 
        let A = Matrix.multiply([rotateTransform, mover]);
        mat3x3Translate(mover, 350, 300);
        A = Matrix.multiply([mover, A]);

        mat3x3Scale(scaleTransform, this.final_sx, this.final_sy);
        mat3x3Translate(mover, -center.x, -center.y);
        let B = Matrix.multiply([scaleTransform, mover]);
        mat3x3Translate(mover, center.x, center.y);
        A = Matrix.multiply([A,B]);

        for (let i = 0; i < ball_points.length; i++){
            ball_points[i] = Matrix.multiply([A, ball_points[i]])

        }
        console.log(this.final_sx - this.final_sx_m);
        if (this.final_sx - this.final_sx_m  > -.03 && this.final_sx - this.final_sx_m <0){
            this.final_color = [Math.floor(Math.random()*255),Math.floor(Math.random()*255),Math.floor(Math.random()*255), 255];
        }
        this.drawConvexPolygon(ball_points, this.final_color);
